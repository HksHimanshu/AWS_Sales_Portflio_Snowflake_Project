...
def main():
    ...
    #Further filter to remove already processed Data in curated Dataset
    curated_df= session.sql("select order_id from sales_dwh.curated.in_sales_order")
    final_sales_df = final_sales_df.join(curated_df,curated_df['order_id'] == final_sales_df['order_id'],join_type='leftanti')
